    ...
    public class ArgumentNode : CommandNode
    {
        ...
        public bool MatchArgument(string arg, bool fuzzy)
        {
            try
            {
                ...
                {
                    // If the "params" keyword is present, an empty entry is acceptable?
                    // Otherwise a string[] requires a value?  Suck it and see!
                    if (Param.IsDefined(typeof(ParamArrayAttribute)))
                    {
                        Console.WriteLine($"{Param.Name} says yes to '{arg}'!");
                        return true;
                    }
                ...
                Console.WriteLine($"   Type is {Param.ParameterType}, ParamArrayAttribute is {Param.IsDefined(typeof(ParamArrayAttribute), false)}");
                return false;
            }
            catch
            {
